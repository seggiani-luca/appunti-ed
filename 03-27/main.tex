...
Nel caso ci fossero ulteriori dubbi su questo metodo, si rimanda alla dispensa in \url{https://raw.githubusercontent.com/seggiani-luca/appunti-ed/4787dc2e5ce5dc6eb266299cac64ee000cebf4dc/apropos/common_emitter/common_emitter.pdf}.
...
\subsubsection{Note sugli amplificatori}
Facciamo quindi una trattazione dettagliata di cos'è un \textit{amplificatore} in primo luogo. Di base, un amplificatore sarà un dispositivo elettronico formato da due porte:
\begin{itemize}
	\item Una porta sarà rivolta verso un generatore di \textit{segnale} $V_s$, con la sua resistenza $R_s$. Solitamente il generatore di segnale è piccolo (rappresenta il segnale che vogliamo amplificare). Chiamiamo le grandezze su questa porta $V_i$ e $i_i$;
	\item L'altra portà sara rivolta verso un \textit{carico}, rappresentato dall'impedenza di carico $R_L$. Questa vorrà vedere una versione amplificata del segnale erogato da $V_s$. Esistono diversi tipi di amplificazione, fra cui amplificazione di \textit{tensione} e amplificazione di \textit{corrente}. In generale, comunque, vogliamo avere un amplificazione della \textit{potenza} al carico, da:
$$
P = VI = \frac{V^2}{R} = I^2 R
$$
Chiamiamo le grandezze su questa porta $V_o$ e $i_o$;
\end{itemize}

Ci deve essere chiaro che un amplificatore è sempre un componente \textit{attivo}, cioè oltre alle porte di ingresso e di uscita di cui abbiamo parlato, dovra essere connesso all'alimentazione con $V_{CC}$ e $V_{EE}$. 

Definiamo diversi parametri riguardo agli amplificatori, fra cui:
\begin{itemize}
\item \textbf{\textsf{Guadagno di tensione}} $A_V$ \\
Questa equivarrà all'aumento di tensione che vediamo sul carico rispetto al segnale, cioè:
$$
A_V = \frac{V_o}{V_i} \Big|_{R_L \rightarrow +\infty}
$$
Il limite della resistenza ad infinito significa che sostanzialmente prendiamo la tensione imposta alla porta di uscita quasi fosse un aperto;

\item \textbf{\textsf{Guadagno di corrente}} $A_i$ \\
Allo stesso modo, possiamo definire l'aumento di corrente che vediamo sul carico rispetto al segnale, cioè:
$$
A_i = \frac{i_o}{i_i} 
$$

\item \textbf{\textsf{Impedenza di ingresso}} $R_i$ \\
Questa sarà la resistenza (generalizzata ad impedenza per segnali non DC) vista all'ingresso dell'amplificatore, e cioè:
$$
R_i = \frac{V_i}{i_i}
$$

\item \textbf{\textsf{Impedenza di uscita}} $R_o$ \\
Infine, questa sarà la resistenza (generalizzata ad impedenza per segnali non DC) vista all'uscita dell'amplificatore, e cioè:
$$
R_o = \frac{V_o}{i_o} \Big|_{V_s = 0}
$$
Notiamo che proviamo questa impedenza staccando il generatore di segnale alla porta di ingresso.
...
Consideriamo allora la seguente rete (a 4 resistori):

\begin{circuitikz}
% Left circuit: voltage divider bias
\draw
...
% Right circuit: single resistor bias
\begin{scope}[xshift=10cm]
\draw
(0,0) node[ground]{} to[voltage source, V=$V_{BB}$] (0,3)
to[R=$R_B$] (2,3);

\draw (2,3) node[npn, anchor=B] (Q2) {};

\draw (Q2.C) to[R=$R_C$] ++(0,3) node[vcc]{};
\draw (Q2.E) to[R=$R_E$] ++(0,-3) node[ground]{};

\draw[->] (1.4,3.3) -- (2,3.3) node[midway, above]{$I_B$};
...
Questo circuito è il primo tipo di amplificatore che vedremo, basato sul transistor di tipo BJT ad \textit{emettitore comune} (che avevamo iniziato a vedere in 10.1.4).

Il segnale si va ad immettere sul nodo centrale alle due resistenze $R_1$ ed $R_2$, cioè alla base del BJT. Solitamente, si usa l'accoppiamento in AC (anziché in DC, in quanto ci permette di non modificare il punto di riposo): vedremo in 12.2 cosa si intende di preciso.
L'idea del partitore formato da $R_1$ e $R_2$ è quello di stabilire un \textit{bias}, cioè uno scostamento fisso della tensione alla base del transistor che determina il punto di riposo o \textit{quiescenza} $Q$ del circuito. A questo sommeremo un \textit{piccolo} voltaggio di segnale $V_s$ che verrà effettivamente amplificato.
...
Questo ci darà la topologia a destra, stabilita la resistenza vista $R_B$ e la tensione $V_{BB}$ all'ammissione dell'amplificatore (il cosiddetto bias che stavamo cercando).
...
$$
che sotto determinate ipotesi ci dà il risultato:
$$
\begin{cases}
	V_{BB} >> V_\gamma \\
