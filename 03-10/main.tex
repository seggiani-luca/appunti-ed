...
$$
\vec{J}_{P, \text{Diffusione}} = - q D_P \frac{\partial p}{\partial x} \hat{i}_x 
$$
	\item Allo stesso modo, la corrente di diffusione degli elettroni liberi sarà:
$$
\vec{J}_{N, \text{Diffusione}} = + q D_N \frac{\partial n}{\partial x} \hat{i}_x 
$$
\end{itemize}
...
\begin{itemize}
	...
	\item Allo stesso modo nella regione N avremo una serie di ioni \textit{donatori} fissi. In condizioni di equilibrio (quindi prima di collegare la regione P), ad ognuno di questi sarà associati un elettrone libero di drogaggio, per cui la carica complessiva sarà anche qui neutra.
\end{itemize}
...
$$
\begin{cases}
	\vec{J}_N = + q D_N \frac{\partial n}{\partial x} \hat{i}_x + n q \mu_N \vec{E} = 0 \\
	\vec{J}_P = - q D_P \frac{\partial p}{\partial x} \hat{i}_x + p q \mu_P \vec{E} = 0
\end{cases}
$$

Si può dare un risultato analitico per $\vec{E}$ risolvendo le equazioni ed applicando il teorema di Gauss. In questi appunti, però, si sceglie di proseguire in maniera \textit{qualitativa}. 
...
$$
E(x) = -\frac{d V(x)}{dx} \implies \Delta V = V_0 = \frac{k_B T}{q} \ln \left( \frac{N_A N_D}{n_i^2} \right)
$$
...
\begin{itemize}
	...
	\item Per quanto riguarda le densità di corrente di \textit{deriva} $\vec{J}_{N, \text{Drift}}$ e $\vec{J}_{P, \text{Drift}}$, invece si avrà che queste sono piccole e restano costanti. 
\end{itemize}
