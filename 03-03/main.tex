...
Ricordiamo che questo deriva dalla formulazione propria della legge di Ohm:
$$
J = \sigma E
$$
...
\begin{theorem}{Legge microscopica di Ohm}
...
Prendiamo ad esempio il silicio, che forma legami di tipo covalenti. Abbiamo detto in 1.2.3 che il silicio presenta 4 elettroni nello strato di valenza, per cui è capace di formare legami covalenti con altri 4 atomi di silicio in una struttura cristallina tidimensionale.
...
Di base, in una situazione dove la temperatura è $T = 0 \, \mathrm{K}$ gradi Kelvin, gli elettroni non sono liberi di muoversi nel reticolo e non si ha conduzione;
	\item 
Se invece si pone $T \neq 0 \, \mathrm{K}$, si può avere conduzione attraverso elettroni liberi che vengono "portati" via dal legame. Questo è reso possibile dal fatto che l'energia fornita come aumento di temperatura all'atomo è tale da superare l'energia di legame, e quindi liberare un elettrone coinvolto nel legame.
\end{itemize}
...
Questo si ricava sperimentalmente come:
$$
n_i = B T^\frac{3}{2} e^{-\frac{E_G}{2 k_B T}}
$$
dove $n_i$ è la densità di elettroni liberi (analoga alla $n$ dei conduttori).

Ad esempio, abbiamo che a $300 \, \mathrm{K}$ gradi Kelvin, il numero di elettroni liberi nel silicio è circa $10^{10} \, \mathrm{cm}^{-3}$.

Diciamo che il silicio è un semiconduttore \textbf{intrinsico} in quanto ha naturalmente bassa conduttività. Di un semiconduttore drogato per ottenere maggiore conduttività si dice che è invece \textbf{estrinseco}.
...
\noindent
Il fenomeno visto finora prende il nome di \textbf{generazione termica} di elettroni liberi, in quanto chiaramente richiede di fornire calore al materiale semiconduttore le cui proprietà di conduzione vogliamo variare.

Si può osservare anche il fenomeno opposto, detto di \textbf{ricombinazione termica}, quando un elettrone perde energia e decade nuovamente in un legame covalente.
...
\noindent
Un fenomeno che non abbiamo visto è quello comportato dal fatto che un elettrone che viene liberato da un legame forma una \textbf{lacuna} o \textit{vacuità} nel reticolo crristallino, cioè una regione di "vuoto" della carica.
...
\begin{itemize}
	\item Gli \textbf{elettroni}, che hanno carica $-1 \, e$;
	\item Le \textbf{lacune}, che hanno carica $+1 \, e$.
\end{itemize}
...
$$
\sigma = n_i \mu_n e + \rho \mu_p e
$$
dove $\mu_p$ è la \textit{mobilità delle lacune}, che notiamo differisce da quella degli elettroni (già introdotta come $\mu_n$).
