...
$$
\phi(x) = \frac{\phi\left(L-x\right)+V_{D}x}{L}, \quad \left\{0\le x\le L\right\}
$$
Sarà che $\phi(x)$ attorno al source (cioè $\phi(0)$) varrà un certo valore di $\phi > 0$, mentre $\phi(x)$ attorno al drain (cioè $\phi(L)$) rispetterà il valore imposto $V_G$. 
...
$$
mentre vicino al dain questa diminuirà fino ad un certo valore:
$$
...
$$
x_p = \frac{\left(V_{G}-V_{T}\right)L-\phi L}{\left(V_{D}-\phi\right)}
$$
