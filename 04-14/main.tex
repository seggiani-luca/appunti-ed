...
$$
Sarà che $\phi(x)$ attorno al source (cioè $\phi(0)$) varrà 0, mentre $\phi(x)$ attorno al drain (cioè $\phi(L)$) rispetterà il valore imposto $V_D$. 
...
Infine, notiamo come la saturazione nel MOSFET ha effettivamente un comportamento macroscopico simile a quella del BJT, ma viene provocata da fenomeni completamente diversi all'interno della struttura del transistore.
...
\begin{enumerate}
	...
	$$
	che vediamo appunto è costane fissata $V_G$ e calcolata $K$ dagli stessi parametri della regione ohmica. Notiamo che per $K$ esistono due convenzioni: uno che la prende come già inclusiva del termine $\frac{1}{2}$, e una che lo lascia fuori. In questi appunti è stata combattuta una battaglia fra me (preferisco lasciare il $\frac{1}{2}$ fuori) e il mio professore (spesso include il $\frac{1}{2}$), per cui la convenzione usata potrebbe cambiare a seconda del contesto. 
\end{enumerate}
...
Tracciando l'andamento sul grafico, al variare della $V_{GS}$, si ha:
	\begin{center}
		\includegraphics[scale=0.28]{../figures/mosfet_char.png}
	\end{center}
